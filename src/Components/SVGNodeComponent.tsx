...
const SVGNode: Component<NodeProps> = (props) => {
  ...
  const handleHover = () => {
    setActiv((isActive) => !isActive);
  };

  const isVisible = () => isActive() && (props.showButtons ?? true)
  ...
  return (
    <g onMouseEnter={() => handleHover()} onMouseLeave={() => handleHover()} data-nodeindex={props.nodeIndex}>
      <circle
      ...
      <Show when={isVisible()}>
        ...
        <g onClick={() => props.onAddLink(props.nodeIndex)}>
          <circle
            cx={northPosition().x}
            cy={northPosition().y}
            r={iconRadius * 0.6}
        ...
        <g onClick={() => props.onAddNode(props.nodeIndex)}>
          <circle
            cx={eastPosition().x}
            cy={eastPosition().y}
            r={iconRadius * 0.4}
